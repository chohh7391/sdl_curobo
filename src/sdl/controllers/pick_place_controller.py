...
from isaacsim.core.api.tasks import PickPlace as BasePickPlace
...
from sdl.tasks.pick_place import CuroboPickPlace


class CuroboPickPlaceController(BaseController):
    def __init__(
        self,
        my_world: World,
        my_task: CuroboPickPlace,
        name: str = "curobo_pick_place_controller",
        ...
        if constrain_grasp_approach:
            pose_metric = PoseCostMetric.create_grasp_approach_metric(
                offset_position=0.0, tstep_fraction=0.8
            )
    ...
    def attach_obj(
        ...
        object_prim_path = self.my_task.object.prim_path
    ...
    def plan(
        ...
        if self._save_log:  # and not result.success.item(): # logging for debugging
            UsdHelper.write_motion_gen_log(
                ...
                cu_js,
                ik_goal,
                join_path("log/usd/", "object") + "_debug",
                write_ik=False,
                write_trajopt=True,
    ...
    def forward(
        ...
        if self.cmd_plan is None:
            ...
            ee_orientation_goal = self.my_task.target_orientation
    ...
    def reached_target(self, observations: dict) -> bool:
        ...
        if np.linalg.norm(
            ...
            if self.my_task.object_in_hand is None:
                print("reached picking target:", self.my_task.object.name)
            else:
                print("reached placing target:", self.my_task.object.name)
            return True
        else:
            return False
    ...
    def set_constraint(self, grasp_axis: str = "z"):

        if grasp_axis.lower() == "z":
            self.plan_config.pose_cost_metric = PoseCostMetric(
                hold_partial_pose=True,
                hold_vec_weight=self.motion_gen.tensor_args.to_device([1, 1, 0, 0, 0, 0]),
            )
        elif grasp_axis.lower() == "xy":
            self.plan_config.pose_cost_metric = PoseCostMetric(
                hold_partial_pose=True,
                hold_vec_weight=self.motion_gen.tensor_args.to_device([1, 0, 1, 0, 0, 0]),
            )
        else:
            raise ValueError("grasp_mode should be only 2 values: 'xy' or 'z'")


    def reset_constraint(self) -> None:

        self.plan_config.pose_cost_metric.reset_metric()