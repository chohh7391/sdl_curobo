...
from sdl.controllers import CuroboPickPlaceController
...
ignore_substring = ["FR5", "material", "Plane"]
my_world = World(stage_units_in_meters=1.0)
stage = my_world.stage
stage.SetDefaultPrim(stage.GetPrimAtPath("/World"))

my_task = CuroboPickPlace(
    name="curobo_pick_place", offset=np.array([0.0, 0.0, 0.0])
)
...
place_positions = np.array([
    [0.5, -0.5, my_task.object_size[2] / 2 + 0.1],
    [0.5, 0.5, my_task.object_size[2] / 2 + 0.1],
    [-0.5, 0.5, my_task.object_size[2] / 2 + 0.1],
    [-0.5, -0.5, my_task.object_size[2] / 2 + 0.1],
])

place_orientation = np.array([1, 0, 0, 0])

observations = my_world.get_observations()
my_task.get_pick_pose(observations)
my_task.set_place_pose(
    position=place_positions[0],
    ...
    if my_world.is_playing():
        ...
        if my_controller.reached_target(observations):
            if my_fr5.gripper.get_joint_positions()[0] > 0.3:  # reached placing target
                ...
                my_task.get_pick_pose(observations)
                my_task.set_place_pose(
                    position=place_positions[i],
                    orientation=place_orientation,
                )
                my_controller.reset_constraint() # reset constraint
                ...
                my_task.get_place_pose(observations)
                my_controller.set_constraint(grasp_axis="z") # add constraint
